module Cms::PageIndexingBaseJob
  ...
  def index(options)
    ...
    @cur_page = @cur_item = criteria.find(options[:id]).becomes_with_route
  ...
  def delete(options)
    ...
    with_rescue(Elasticsearch::Transport::Transport::ServerError) do
      es_client.delete(index: index_name, type: index_type, id: index_item_id(@id))
    end

    if remove_file_ids.present?
      remove_file_ids.uniq.each do |id|
        with_rescue(Elasticsearch::Transport::Transport::ServerError) do
          es_client.delete(index: index_name, type: index_type, id: index_file_id(id))
        end
      end
    end
  end
