class Gws::Share::Apis::FolderListController < ApplicationController
    ...
    items = items.compact.map do |item|
      {
        ...
        depth: item.depth,
        url: item_url(item),
        tree_url: gws_share_apis_folder_list_path(id: item.id, type: @type, category: params[:category].presence || "-"),
  ...
  def item_url(item)
    cate = params[:category].presence || "-"
    return gws_share_folder_files_path(folder: item.id, category: cate) if @type == 'gws/share/files'
    return gws_share_management_folder_files_path(folder: item.id, category: cate) if @type == 'gws/share/management/files'
