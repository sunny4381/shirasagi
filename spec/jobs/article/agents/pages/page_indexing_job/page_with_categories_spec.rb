...
describe Article::Agents::Pages::PageIndexingJob, dbscope: :example do
  ...
  let(:cate_node) { create(:category_node_page, cur_site: site, layout_id: layout.id) }
  ...
  before do
    ...
    page.category_ids = [ cate_node.id ]
    page.save!
  end
  ...
  it do
    ...
    es_requests.first.tap do |request|
      ...
      expect(body['cate_categories']).to include(cate_node.name)
